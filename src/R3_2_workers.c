...
void workersLogic(int fd_leitura, int id, CONFIG *config, int numFIles) {
    ...
    strcpy(message.top_ip, "N/A"); // inicializa o top IP
    
    char ip_list[256][64]; // lista de IPs encontrados
    long ip_count[256];    // contagem de cada IP
    int ip_total = 0;
    ...
    for(int i = 0; i < numFIles; i++){
        ...
        // escreve resultados no ficheiro de resultados
        char nameResult[64];
        snprintf(nameResult, sizeof(nameResult), "results_%d.txt", (int)getpid());
        int fdResult = open(nameResult, O_WRONLY | O_CREAT | O_APPEND, 0644); // O_APPEND para não sobrescrever
        if(fdResult == -1){
    ...
    } // fecha o ciclo for dos ficheiros

    // encontra o top IP depois de processar todos os ficheiros
    long max_count = 0;
    for (int k = 0; k < ip_total; k++) {
        if (ip_count[k] > max_count) {
